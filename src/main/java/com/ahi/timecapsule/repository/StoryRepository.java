...
public interface StoryRepository extends JpaRepository<Story, Long> {
  ...
  @Query("SELECT s FROM Story s WHERE s.isShared = true AND (s.title LIKE CONCAT('%', :keyword, '%') OR s.content LIKE CONCAT('%', :keyword, '%'))")
  Page<Story> findByIsSharedTrueAndTitleContainsOrContentContains(@Param("keyword") String keyword, Pageable pageable);
